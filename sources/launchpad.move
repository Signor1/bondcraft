...
public fun buy_tokens<T>(
    ...
    assert!(
        amount <= MAX_TOKENS_PER_TX * (10u64).pow(launchpad.params.decimals),
        EExcessivePurchase,
    );
    assert!(
    ...
    let current_price = bonding_curve::calculate_price(
        launchpad.state.tokens_sold,
        launchpad.params.k,
    );
...
public fun close_funding<T>(launchpad: &mut Launchpad<T>, ctx: &mut TxContext) {
...
public fun bootstrap_liquidity<T>(
    ...
    let final_price = bonding_curve::calculate_price(
        launchpad.state.tokens_sold,
        launchpad.params.k,
    );
...
public fun claim_creator_tokens<T>(launchpad: &mut Launchpad<T>, ctx: &mut TxContext) {
    assert!(launchpad.creator == tx_context::sender(ctx), EUnauthorized);
    assert!(launchpad.state.phase >= PHASE_CLOSED, EInvalidPhase);
...
public fun claim_platform_tokens<T>(launchpad: &mut Launchpad<T>, ctx: &mut TxContext) {
...
public fun withdraw_funding<T>(launchpad: &mut Launchpad<T>, amount: u64, ctx: &mut TxContext) {
    assert!(launchpad.creator == tx_context::sender(ctx), EUnauthorized);
    assert!(launchpad.state.phase >= PHASE_CLOSED, EInvalidPhase);
