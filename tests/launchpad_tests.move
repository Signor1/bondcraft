...
fun test_buy_tokens_success() {
    ...
    let cost = bonding_curve::calculate_cost(0, PURCHASE_AMOUNT, k);
...
fun test_buy_tokens_insufficient_payment() {
    ...
    let ctx = ts::ctx(&mut scenario);
    let payment = coin::mint_for_testing<USDC>(1000, ctx); // Too little
...
fun test_funding_goal_reached() {
    ...
    ts::next_epoch(&mut scenario, buyer);
    let ctx = ts::ctx(&mut scenario);
    launchpad::buy_tokens(&mut launchpad, payment, FUNDING_TOKENS, ctx);

    // Verify phase changed to CLOSED
    assert!(launchpad::phase(&launchpad) == 1, 0); // PHASE_OPEN
    assert!(launchpad::vesting_start_epoch(&launchpad) > 0, 1); // PHASE_CLOSED
...
fun test_bootstrap_liquidity() {
    ...
    let treasury = scenario.take_from_sender<TreasuryCap<TESTTOKEN>>();
    let metadata = scenario.take_from_sender<CoinMetadata<TESTTOKEN>>();
    let usdc_metadata = scenario.take_from_sender<CoinMetadata<USDC>>();
    ...
    let (_admin_cap, cetus_config) = config::new_global_config_for_test(ctx, 10);
    let mut pool = factory::new_pools_for_test(ctx);
    let clock = clock::create_for_testing(ctx);
    launchpad::bootstrap_liquidity(
        &mut launchpad,
        &cetus_config,
        &mut pool,
        &metadata,
    ...
    ts::return_to_sender(&scenario, launchpad);
    ts::return_to_sender(&scenario, metadata);
    ts::return_to_sender(&scenario, usdc_metadata);
    ts::return_to_sender(&scenario, _admin_cap);
    ts::return_to_sender(&scenario, cetus_config);
    ts::return_to_sender(&scenario, pool);
    ts::return_to_sender(&scenario, clock);
    ts::end(scenario);
}
...
fun test_multiple_purchases() {
    ...
    let k = launchpad::k(&launchpad);
    let cost1 = bonding_curve::calculate_cost(0, PURCHASE_AMOUNT, k);
